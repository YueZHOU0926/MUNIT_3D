...
############## Can already support 3D, but need for later check###########
...
#########Here only related with Encoder and Decoder, since modified before, may not have to change##########
#####need to check later#######
class VAEGen(nn.Module):
    ...
    def forward(self, volume):
        ...
        hiddens = self.encode(volume)
        if self.training == True:
            noise = Variable(torch.randn(hiddens.size()).cuda(hiddens.data.get_device()))
            volume_recon = self.decode(hiddens + noise)
        else:
            volume_recon = self.decode(hiddens)
        return volume_recon, hiddens

    def encode(self, volume):
        hiddens = self.enc(volume)
        noise = Variable(torch.randn(hiddens.size()).cuda(hiddens.data.get_device()))
        return hiddens, noise

    def decode(self, hiddens):
        volume = self.dec(hiddens)
        return volume
...
######Need more check#######
####change image to volume might be better#####
...
####### Do not need to modify to 3D as long as Conv2dBlock is correctly changed########
class ResBlock(nn.Module):
    def __init__(self, dim, norm='in', activation='relu', pad_type='replicate'):
...
####### Not sure if need to modify to 3D, may have to check a bit how the normalization works ########
...
####### !!!Need to modify to 3D, but seems it can adapt 3D without changing########
class AdaptiveInstanceNorm3d(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply instance norm
        ######### Can modify better if use nn.InstanceNorm? new version support this###########
...
####### !!!Need to modify to 3D, but in the whole pipeline, I didn't fine this class Vgg16 #######
####### in the network, what they used is to load a pretrained VGG net, and add a vgg_loss #######
####### when traing, but we do not have a vgg_pretrained_model for 3D, do we??             #######                   
